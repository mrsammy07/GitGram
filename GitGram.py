...
"""
    ...
    if data.get('forkee'):
        response = post_tg(
            groupid,
            f"🙇 <a href='{data['sender']['html_url']}'>{data['sender']['login']}</a> <a>forked<a> <a href='{data['repository']['html_url']}'>{data['repository']['name']}</a>\n<a>Total forks<a> :⚡ {data['repository']['forks_count']}",
            "html")
        return response
